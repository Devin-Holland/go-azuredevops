// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"strconv"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/swag"
)

// TestPlanHubData test plan hub data
// swagger:model TestPlanHubData
type TestPlanHubData struct {

	// selected suite Id
	SelectedSuiteID int32 `json:"selectedSuiteId,omitempty"`

	// test plan
	TestPlan *TestPlan `json:"testPlan,omitempty"`

	// test points
	TestPoints []*TestPoint `json:"testPoints"`

	// test suites
	TestSuites []*TestSuite `json:"testSuites"`

	// total test points
	TotalTestPoints int32 `json:"totalTestPoints,omitempty"`
}

// Validate validates this test plan hub data
func (m *TestPlanHubData) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateTestPlan(formats); err != nil {
		res = append(res, err)
	}

	if err := m.validateTestPoints(formats); err != nil {
		res = append(res, err)
	}

	if err := m.validateTestSuites(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *TestPlanHubData) validateTestPlan(formats strfmt.Registry) error {

	if swag.IsZero(m.TestPlan) { // not required
		return nil
	}

	if m.TestPlan != nil {
		if err := m.TestPlan.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("testPlan")
			}
			return err
		}
	}

	return nil
}

func (m *TestPlanHubData) validateTestPoints(formats strfmt.Registry) error {

	if swag.IsZero(m.TestPoints) { // not required
		return nil
	}

	for i := 0; i < len(m.TestPoints); i++ {
		if swag.IsZero(m.TestPoints[i]) { // not required
			continue
		}

		if m.TestPoints[i] != nil {
			if err := m.TestPoints[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("testPoints" + "." + strconv.Itoa(i))
				}
				return err
			}
		}

	}

	return nil
}

func (m *TestPlanHubData) validateTestSuites(formats strfmt.Registry) error {

	if swag.IsZero(m.TestSuites) { // not required
		return nil
	}

	for i := 0; i < len(m.TestSuites); i++ {
		if swag.IsZero(m.TestSuites[i]) { // not required
			continue
		}

		if m.TestSuites[i] != nil {
			if err := m.TestSuites[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("testSuites" + "." + strconv.Itoa(i))
				}
				return err
			}
		}

	}

	return nil
}

// MarshalBinary interface implementation
func (m *TestPlanHubData) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *TestPlanHubData) UnmarshalBinary(b []byte) error {
	var res TestPlanHubData
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
